...
def build_model_lstm(input_shape, output_layers=1, return_seq=False):
    ...
    # 2 LSTM layers
    model.add(keras.layers.LSTM(256, input_shape=input_shape, return_sequences=True, stateful=False, kernel_regularizer=regularizers.l2(0.01)))#, dropout=0.2))#, recurrent_regularizer='l1'))
    model.add(keras.layers.Dropout(0.2))  # luiz
...
def train_model(features, labels, arquitetura, seq_lenght, output_layers=1, texto='', return_seq=False):
    ...
    checkpoint_path = f'{MODELS_DIR}/models-{datahora}'
    ...
    backup_callback = tf.keras.callbacks.ModelCheckpoint(checkpoint_path, monitor = 'val_loss', verbose=1, save_best_only=True, mode='min')
    history = model.fit(X_train, y_train, validation_data=(X_validation, y_validation), batch_size=32, epochs=10, callbacks=[backup_callback], verbose=1, shuffle=True)
