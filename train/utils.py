...
SEGMENTS_PER_PERSON = config.SEGMENTS_PER_PERSON  #7500 raws in the original set by sample (pearson)
RECOLA_SUBJECTS = config.RECOLA_SUBJECTS  # 18 subjects
...
def prepare_datasets(X, y, validation_size, test_size, output_layers):
    ...
    train_split = SEGMENTS_PER_PERSON * (RECOLA_SUBJECTS - validation_size - test_size)  #(90000)-67%
    validation_split = SEGMENTS_PER_PERSON * validation_size  #(22500)-17%
    test_split = SEGMENTS_PER_PERSON * test_size  #(22500)-17%

    #não é possível usar train_test_split com porcentagens para garantir sequencias da mesma pessoa
    #create train, validation and test split
    X_test, y_test =  X[:test_split], y[:test_split]  # 0 to 22500 (exclusive)
