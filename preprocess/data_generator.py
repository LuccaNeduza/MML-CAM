...
def get_samples(subject_id):
    """Recupera audio and video data e o label de cada subsample de 40ms da base de dados
    ...
    arousal_audio_features_path = RECOLA_DIR / 'features_audio/arousal/{}.arff'.format(subject_id)
    valence_audio_features_path = RECOLA_DIR / 'features_audio/valence/{}.arff'.format(subject_id)

    # load audio features file
    arousal_audio_feature = np.array(arff.load(open(arousal_audio_features_path, 'r'))['data'])
    valence_audio_feature = np.array(arff.load(open(valence_audio_features_path, 'r'))['data'])
    ...
    arousal_video_features_path = RECOLA_DIR / 'features_video_appearance/arousal/{}.arff'.format(subject_id)
    valence_video_features_path = RECOLA_DIR / 'features_video_appearance/valence/{}.arff'.format(subject_id)

    # load audio features file
    arousal_video_feature = np.array(arff.load(open(arousal_video_features_path, 'r'))['data'])
    valence_video_feature = np.array(arff.load(open(valence_video_features_path, 'r'))['data'])
    arousal_video_feature = arousal_video_feature[:, 1:].astype(float)
    valence_video_feature = valence_video_feature[:, 1:].astype(float)

    return arousal_audio_feature, valence_audio_feature, arousal_video_feature, valence_video_feature, arousal_label, valence_label, np.dstack([arousal_label, valence_label])[0].astype(np.float32)


def serialize_sample_json(data, subject_id, debug=True):

    for i, (arousal_audio_feature, valence_audio_feature, arousal_video_feature, valence_video_feature, arousal_label, valence_label, labels) in enumerate(zip(*get_samples(subject_id))):
        ...
        del arousal_audio_feature, valence_audio_feature, arousal_video_feature, valence_video_feature, arousal_label, valence_label


def data_generation():

    print('Starting preprocessing audio and video data and saving segmented information...')
